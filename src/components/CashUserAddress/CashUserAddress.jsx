...
import styles from "./CashUserAddress.module.css"
import { useFormik } from 'formik'
import * as Yup from 'yup';
import { useCashOrder } from '../../hooks/ordersHooks';
import ErrorAlert from '../common/ErrorAlert/ErrorAlert';
import { useLocation, useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';
export default function CashUserAddress() {
    const location = useLocation();
  const cartId = location.state?.cartId;
const navigate=useNavigate()
  
  const { mutate: cashorder, isError, isPending, error } = useCashOrder();

  
  const handleSubmit = (values) => {
    cashorder({ cartId, shippingAddress:{...values} }, {
      onSuccess: () => {
